...
function my_preg_replace($pattern, $replacement, $source)
{

            $pattern     = '/<!--\s#BeginLibraryItem\s\"\/(.*?)\"\s-->.*?<!--\s#EndLibraryItem\s-->/s';
            $replacement = "'{include file='.strtolower('\\1'). '}'";
           // $source      = preg_replace_callback($pattern, function($r) {echo $r[1]; return '{include file='.strtolower($r[1]).'}'; }, $subject);
